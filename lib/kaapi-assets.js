...
// Modular: future flavors swap this file only, engine stays untouched.

export const KAAPI_CACHE = {
  db: 'kaapi-scroll',
  store: 'frames',
  // Bump on any renderer/asset change to invalidate cached WebP frames.
  version: 'v1.3.1-kaapi-real',
};
...
// Two photorealistic Kaapi Da product renders.
// Served locally from /public so canvas is same-origin and never taints.
export const KAAPI_IMAGES = {
  exploded: '/kaapi/intact.png',   // NOTE: file names on disk are swapped; this file holds the exploded scene (16:9)
  intact:   '/kaapi/exploded.png', // this file holds the pristine sealed can + reflection (~4:3)
};

// Which source image each clip uses.
export const KAAPI_CLIPS = {
  deconstruct: { image: 'exploded', kind: 'reveal' },      // reveal-from-bottom animation
  orbit:       { image: 'intact',   kind: 'orbit'  },      // fake horizontal rotation
  bridge:      { image: 'intact',   kind: 'bridge' },      // pan-in with tilt
  detail:      { image: 'intact',   kind: 'detail' },      // macro push-in
};
